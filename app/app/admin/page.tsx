...
import { ReportAdminTab } from '@/components/admin/ReportAdminTab';
...
export default function AdminPage() {
  ...
  // Handlers สำหรับ Report Tab
  const [reportOpenDialog, setReportOpenDialog] = useState<(() => void) | null>(null);
  const [reportRefresh, setReportRefresh] = useState<(() => void) | null>(null);
  ...
  const loadData = async () => {
    // ถ้าเป็น Tab รายงาน ให้เรียก refresh ของ ReportAdminTab
    if (activeTab === 1 && reportRefresh) {
      reportRefresh();
      return;
    }
  ...
  const handleOpenDialog = (item?: any) => {
    // ถ้าเป็น Tab รายงาน ให้เรียก dialog ของ ReportAdminTab
    if (activeTab === 1 && reportOpenDialog) {
      reportOpenDialog();
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isServiceDialog) {
        ...
        // Report - handled by ReportAdminTab, should not reach here
        console.error('Report save should be handled by ReportAdminTab');
        return;
      } else if (activeTab === 2) {
  ...
  const getDialogTitle = () => {
    ...
    const labels = ['ชุดข้อมูล', 'รายงาน', 'หน่วยงาน', 'นโยบายและแผนความมั่นคง'];
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">จัดการชุดข้อมูล บริการ หน่วยงาน นโยบาย และรายงาน</p>
        </div>
      ...
      <Tabs
        ...
        onChange={(_, v) => {
          setActiveTab(v);
          // Update URL with tab parameter immediately
          const newUrl = `/datacatalog/app/admin?tab=${v}`;
      ...
      {activeTab === 1 && user && (
        <ReportAdminTab
          userId={user.id}
          onReady={(openDialog, refresh) => {
            setReportOpenDialog(() => openDialog);
            setReportRefresh(() => refresh);
          }}
        />
      )}
